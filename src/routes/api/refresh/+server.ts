...
export const POST = async ({ cookies, request }: RequestEvent) => {
	...
	try {
		...
		if (claims) {
			...
			try {
				...
				const newAccessToken = jwt.sign({ authedUser: refreshClaims.authedUser }, SECRET_ACCESS, {
					expiresIn: '10m'
				});
				...
				if (error.name === 'JsonWebTokenError') {
					return new Response(
						JSON.stringify({
							error: true,
							success: false,
							message: 'Unable to verify comms!',
							data: undefined
						}),
						{ status: 500 }
					);
		...
		if (error.name === 'TokenExpiredError') {
			return new Response(
				JSON.stringify({
					error: true,
					success: false,
					message: 'Comms Expired, try again!',
					data: undefined
				}),
				{ status: 401 }
			);
	...
	return new Response(
		JSON.stringify({ error: true, success: false, message: 'Unknown error', data: undefined }),
		{ status: 500 }
	);
};
